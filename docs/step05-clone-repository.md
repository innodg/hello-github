...
## 🎯 이 단계에서 배우는 것
GitHub에 만든 저장소(Repository)를 내 컴퓨터로 가져오는 방법입니다. 이 과정을 통해 원격 저장소와 내 컴퓨터 작업 공간을 연결하는 첫 단계를 익힙니다.
...
아래와 같이 내 컴퓨터에서 확인할 수 있다면 저장소를 내 컴퓨터에 성공적으로 복사했습니다.

![클론 이미지](images/step-05-image-07.png)

이제 우리는 GitHub에 있던 원격 저장소를 내 컴퓨터와 연결했고, 실제로 내 컴퓨터에서 파일을 수정할 준비를 마쳤습니다.

다음 단계에서는 이 저장소 안의 파일을 직접 수정하거나 새로 만들어보면서, Git이 변경 사항을 어떻게 인식하는지 배웁니다.
