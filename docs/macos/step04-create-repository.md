...
## 🎯 이 단계에서...
깃허브 데스크탑(GitHub Desktop)을 사용하여 내 컴퓨터(노트북)에서 저장소(Repository)를 만들고, 이를 깃허브(GitHub)에 업로드합니다.
...
내 컴퓨터(노트북) 화면 **맨 위 메뉴 막대**에서 `File` 메뉴를 클릭합니다.
...
"Description" 칸은 선택사항이라 내용을 적지 않아도 괜찮습니다. "Description" 칸에는 어떤 목적의 저장소인지 설명하는 글을 짧게 작성합니다.
...
아래 사진처럼 "Desktop" 폴더를 선택한 후, `열기` 버튼을 클릭합니다(다른 폴더를 선택 가능하지만 이번 시간에는 "Desktop" 폴더에 저장소를 만들도록 하겠습니다).
...
이제 우리 컴퓨터(노트북)에 만든 저장소를 깃허브에 업로드해보겠습니다!
...
아래 사진처럼 **"Also move this repository to Trash"**(휴지통으로 이동) 박스를 클릭해 체크함으로써 내 컴퓨터(노트북)에서 저장소를 완전히 삭제하도록 설정합니다. 그리고 `Remove` 버튼을 클릭합니다.
...
> **왜 삭제하나요?**
>
> 방금 우리는 저장소를 깃허브에 업로드했으니 언제라도 되찾을 수 있습니다. 다음 단계에서 깃허브에 있는 저장소를 내 컴퓨터로 복제해오는 연습을 할 예정이라 삭제했습니다.
